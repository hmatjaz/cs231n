...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
  ...
  then batch normalization, finally followed by a ReLU.
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    caches.append(c)
    scores, c = affine_forward(H, W2, b2)
    caches.append(c)
    ...
    # Backpropagation
    dH, dW2, db2 = affine_backward(dscores, caches.pop())
    _, dW1, db1 = affine_relu_backward(dH, caches.pop())

    # Store gradient and account for regularization
    grads['W1'] = dW1 + reg * W1
    grads['b1'] = db1
    grads['W2'] = dW2 + reg * W2
    grads['b2'] = db2

    # NOTE: We don't really need to average the loss or gradient as it's already
    # taken care of in the softmax_loss function.
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    # Batchnorm scale & shift parameters
  ...
  def loss(self, X, y=None):
    ...
    # Intermediate variable for previous layer output (start with input X)
    out = X
    ...
    for i in xrange(1, last_layer):
      if self.use_batchnorm:
        out, c = affine_bn_relu_forward(out, params['W%d' % i], params['b%d' % i],
                                        params['gamma%d' % i], params['beta%d' % i],
                                        self.bn_params[i])
      else:
        out, c = affine_relu_forward(out, params['W%d' % i], params['b%d' % i])
      caches.append(c)
    ...
    # Last layer: calculate scores
    scores, c = affine_forward(out, params['W%d' % last_layer], params['b%d' % last_layer])
    caches.append(c)
    ...
    # Account for regularization
    for i in xrange(1, last_layer):
      loss += 0.5 * reg * np.sum(params['W%d' % i] ** 2)

    # Output layer backpropagation
    dout, dW, db = affine_backward(dscores, caches.pop())
    grads['W%d' % last_layer] = dW + reg * params['W%d' % last_layer]
    grads['b%d' % last_layer] = db
    
    # Hidden layers backpropagation
    for i in reversed(xrange(1, last_layer)):
      ...
      # Store and account for regularization
      grads['W%d' % i] = dW + reg * params['W%d' % i]
      grads['b%d' % i] = db
