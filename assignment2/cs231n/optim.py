...
def rmsprop(x, dx, config=None):
  ...
  # Update cache and update weights
...
def adam(x, dx, config=None):
  ...
  # Moving average of gradient: m
  m = beta1 * m + (1 - beta1) * dx

  # Moving average of gradient squared: v
  v = beta2 * v + (1 - beta2) * (dx**2)

  # Correct for biases (see Adam paper)
  m_c = m / (1 - beta1**t)
  v_c = v / (1 - beta2**t)

  # Update weights
  next_x = x - (learning_rate * m_c / (np.sqrt(v_c) + eps))
